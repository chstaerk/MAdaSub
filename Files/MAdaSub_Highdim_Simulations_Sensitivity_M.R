...
Ms <- rev(c(250, 500, 1000, 2500, 5000))  #rev(c(50, 250, 500, 2000, 5000, 12500)) 
burnin_rounds <- 50000/Ms/2 #c(2, 5, 10, 25, 50, 100, 125, 500) 
repeats <- 50000/Ms #c(4, 10, 20, 50, 100, 200, 250, 1000)
Iterations <- Ms[1] * repeats[1] 
burnin <- Ms[1] * burnin_rounds[1] 
nb.k <- 5 
numCores <- 5 # nb.k
...
Times_MAdaSub_parallel <- matrix(NA, N_sim, length(Ms))
Times_MC3 <- rep(NA, N_sim)

Estimates_MAdaSub_parallel <- array(NA, dim=c(N_sim, p, length(Ms)))
Estimates_MC3 <- matrix(NA, N_sim, p)

Accrate_MAdaSub_parallel <- matrix(NA, N_sim, length(Ms))
...
for (i in 1:N_sim){
  ...
  for (m in 1:length(Ms)) {
    ...
    start.time <- Sys.time()
    output_parallel = MAdaSub_parallel(data=data,nb.k=nb.k,M=Ms[m],repeats=repeats[m],priormean=priormean_parallel,L=L_parallel,const=const,savings=savings,family=family,epsilon=epsilon,priorprob=priorprob,prior=prior,g=g,hyper=FALSE,a_prior=NULL,b_prior=NULL,
                             frac.par.update=1, burnin_rounds=burnin_rounds[m], numCores = numCores) 
    end.time <- Sys.time()
    Times_MAdaSub_parallel[i,m] <- difftime(end.time, start.time, unit = "sec")
    Estimates_MAdaSub_parallel[i,,m] <-   apply(output_parallel$importance.individual[repeats[m]+1,,], 2, mean)
    Accrate_MAdaSub_parallel[i,m] <- mean(output_parallel$acc[(burnin_rounds[m]+1):repeats[m],] / Ms[m])
  }
  ...
  print(paste("Acceptance rate MAdaSub parallel M=", Ms , ":",  Accrate_MAdaSub_parallel[i,]))
  print(paste("Acceptance rate MC3:", Accrate_MC3[i]))
  print("#######################################")
}

results = list(Estimates_MAdaSub_parallel = Estimates_MAdaSub_parallel,
...
save(results, file=paste("MAdaSub_results_highdim_Ms_sim_n", n, "_p", p, "_SNR", SNR, "_CPU", nb.k, ".RData", sep=""))
