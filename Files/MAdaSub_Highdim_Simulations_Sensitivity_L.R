...
Ljs <- c(p/500, p/100, p/10, p, 10*p, 100*p)
#Ljs <- c(p/20, p/5, p, 5*p, 20*p)
#Ljs <- c(p/20, p/10, p/5, p/2, p, 2*p, 5*p, 10*p, 20*p)

Times_MAdaSub <- matrix(NA, N_sim, length(Ljs))
Times_MAdaSub_parallel <- matrix(NA, N_sim, length(Ljs))
Times_MC3 <- rep(NA, N_sim)

Estimates_MAdaSub <- array(NA, dim=c(N_sim, p, length(Ljs)))
Estimates_MAdaSub_parallel <- array(NA, dim=c(N_sim, p, length(Ljs)))
Estimates_MC3 <- matrix(NA, N_sim, p)

Accrate_MAdaSub <- matrix(NA, N_sim, length(Ljs))
Accrate_MAdaSub_parallel <- matrix(NA, N_sim, length(Ljs))
...
for (i in 1:N_sim){
  ...
  priormean_parallel <- q/p
  
  for (l in 1:length(Ljs)) {
    start.time <- Sys.time()
    output = mclapply(rep(Iterations, nb.k), MAdaSub, mc.cores = numCores, data=data, priormean=priormean_serial, L=Ljs[l],const=const,savings=savings,family=family,epsilon=epsilon,priorprob=priorprob,prior=prior,g=g,
    ...
    Times_MAdaSub[i,l] <- difftime(end.time, start.time, unit = "sec")
    Estimates_MAdaSub[i,,l] <- colMeans(MAdaSub_estimates)
    Accrate_MAdaSub[i,l] <- mean(MAdaSub_accrates)
  }
  ...
  start.time <- Sys.time()
  output_parallel = MAdaSub_parallel(data=data,nb.k=nb.k,M=M,repeats=repeats,priormean=priormean_parallel,L=Ljs[l],const=const,savings=savings,family=family,epsilon=epsilon,priorprob=priorprob,prior=prior,g=g,hyper=FALSE,a_prior=NULL,b_prior=NULL,
                             frac.par.update=1, burnin_rounds=burnin_rounds, numCores = numCores) 
  end.time <- Sys.time()
  Times_MAdaSub_parallel[i,l] <- difftime(end.time, start.time, unit = "sec")
  Estimates_MAdaSub_parallel[i,,l] <-   apply(output_parallel$importance.individual[repeats+1,,], 2, mean)
  Accrate_MAdaSub_parallel[i,l] <- mean(output_parallel$acc[(burnin_rounds+1):repeats,] / M)
  ...
  #print("First 20 estimates MAdaSub serial")
  #print(Estimates_MAdaSub[i,1:20])
  #print("First 20 estimates MC3")
  #print(Estimates_MC3[i,1:20])
  print(paste("Acceptance rate MAdaSub parallel L=", Ljs , ":",  Accrate_MAdaSub_parallel[i,]))
  print(paste("Acceptance rate MAdaSub serial L=", Ljs , ":",  Accrate_MAdaSub[i,]))
  #print(paste("Acceptance rate MAdaSub serial:", Accrate_MAdaSub_serial[i]))
...
save(results, file=paste("MAdaSub_results_highdim_Ljs_sim_n", n, "_p", p, "_SNR", SNR, "_CPU", nb.k, "_Rep", repeats, "_M", M, ".RData", sep=""))
