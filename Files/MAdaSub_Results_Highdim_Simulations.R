...
epsilon = 1/p
load(paste("./Files/R_simulation_saves/MAdaSub_results_highdim_sim_n", n, "_p", p, "_SNR", SNR, "_CPU", nb.k, "_Rep", repeats, "_M", M, ".RData", sep=""))
...
median(r_hat[highest_PIP], na.rm=TRUE)
median(r_hat_parallel[highest_PIP], na.rm=TRUE)

median(results$Accrate_MAdaSub)
median(results$Accrate_MAdaSub_parallel)
median(results$Accrate_MC3)

#median(results$Times_MAdaSub_parallel)

median(r_hat, na.rm=TRUE)
